...
export const aiService = {
  ...
  // ==================== CONTENT WRITER ====================
  async generateContent(topic: string, tone: string = 'professional', language: string = 'en'): Promise<string> {
    try {
      const prompt = `Write a professional ${tone} article about: ${topic}.\nLanguage: ${langName(language)}.\nLength: 300-400 words.`;
  ...
  // ==================== BLOG GENERATOR ====================
  async generateBlog(title: string, description: string, language: string = 'en'): Promise<string> {
    try {
      const prompt = `Write a complete blog post in ${langName(language)}:\nTitle: ${title}\nDescription: ${description}\n\nFormat:\n- Introduction (2-3 paragraphs)\n- Body (4-5 main points)\n- Conclusion (2-3 paragraphs)\n\nTotal: 800-1000 words`;
      return await generateText(prompt);
    } catch (error) {
  ...
  // ==================== EMAIL GENERATOR ====================
  async generateEmail(type: string, details: string, language: string = 'en'): Promise<string> {
    try {
      const prompt = `Generate a professional ${type} email in ${langName(language)}:\nDetails: ${details}\n\nInclude:\n- Professional subject line\n- Greeting\n- Body (2-3 paragraphs)\n- Call to action\n- Sign-off`;
      return await generateText(prompt);
    } catch (error) {
  ...
  async generateFacebookPost(topic: string, tone: string = 'casual', language: string = 'en'): Promise<string> {
    try {
      const prompt = `Create a ${tone} Facebook post in ${langName(language)} about: ${topic}\n\nTips:\n- Keep it engaging and shareable\n- Use line breaks for readability`;
      return await generateText(prompt);
    } catch (error) {
  ...
  async generateYoutubeScript(topic: string, duration: string = '10 minutes', language: string = 'en'): Promise<string> {
    try {
      const prompt = `Write a ${duration} YouTube video script in ${langName(language)} about: ${topic}\n\nFormat:\n- Hook (first 5 seconds)\n- Introduction\n- Main Content (sections)\n- Call to Action\n- Outro`;
      return await generateText(prompt);
    } catch (error) {
