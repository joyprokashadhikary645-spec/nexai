...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (v.toolType) {
      case 'content-writer':
        rawContent = await aiService.generateContent(v.topic, v.tone, v.language, v.includeHashtags, v.includeEmojis);
        break;
      case 'blog-generator':
        rawContent = await aiService.generateBlog(v.topic, v.description, v.language, v.includeHashtags, v.includeEmojis);
        break;
      case 'email-generator':
        rawContent = await aiService.generateEmail(v.tone, `${v.topic}. ${v.description}`, v.language, v.includeHashtags, v.includeEmojis);
        break;
      case 'facebook-post':
        rawContent = await aiService.generateFacebookPost(v.topic, v.tone, v.language, v.includeHashtags, v.includeEmojis);
        break;
      case 'instagram-caption':
        ...
        break;
      case 'youtube-script':
        rawContent = await aiService.generateYoutubeScript(v.topic, v.description || '10 minutes', v.language, v.includeHashtags, v.includeEmojis);
        break;
    ...
    // Hashtag/Emoji এখন AI নিজেই টপিক অনুযায়ী প্রাসঙ্গিকভাবে বসিয়ে দেয় (প্রম্পটের মাধ্যমে),
    // তাই আগের মতো fixed/canned লিস্ট থেকে আলাদা করে জুড়ে দেওয়ার দরকার নেই — user যা টাইপ করেছে
    // শুধু সেটার ওপর ভিত্তি করেই পুরো আউটপুট তৈরি হয়।
